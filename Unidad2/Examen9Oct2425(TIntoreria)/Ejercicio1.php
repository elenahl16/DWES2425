...
<body>
    ...
    <?php
    //Esto lo que hace es comprobar si se a pulsado guardar
    if (isset($_POST['guardar'])) {

        //si el campo fecha,nombre,tipo y importe esta vacio nos muestra un mensaje diciendo que hay que rellenar todos los datos
        ...
        //if(isset($_POST["tipo"]) and $_POST['tipo']=="Cuero" and isset($_POST['servicios']) and in_array('Planchado',$_POST['servicios'])){ (OTRA FORMA DE PONERLO)

        if(isset($_POST["tipo"]) and $_POST['tipo']=="Cuero" and isset($_POST['servicios']) and in_array('planchado',$_POST['servicios'])){
            //foreach($_POST['servicios'] as $s){(OTRA FORMA DE HACERLO)
              //  if($s=='Planchado'){
        ...
        if(!isset($error)){ // si no esta declarado la variable error nos va a mostrar todos los datos
